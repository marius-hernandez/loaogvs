...
if (isset($_POST['search'])) {
    ...
    $sql="SELECT * FROM teacher WHERE type='teacher' AND (user_year LIKE '%$search%' OR user_id LIKE '%$search%' OR user_first LIKE '%$search%' OR user_last LIKE '%$search%' OR user_middle LIKE '%$search%' OR user_suffix LIKE '%$search%' OR user_email LIKE '%$search%')";
            ...
            while ($row = mysqli_fetch_assoc($result)) {
                ...
                <p id='student'>
                    <h3 name='username' id='username'>
                        <h3>".$row['user_year']. " " .$row['user_id']."</h3>
