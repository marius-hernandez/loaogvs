...
if ($grading=="first") {
    $grading= "Third Quarterly Grade";
    $gradingetc= "Third Quarterly Academic Achievement/Progress Report";
    $sqlstudent = "SELECT * from ".$section."_grade_sec_first WHERE student_first='$first' AND student_last='$last'";
    $stmt=mysqli_stmt_init($conn);
    ...
    */
    ...
    $pdf->MultiCell(25, 6.2, 'Contemporary Philippine Arts from the Regions', 1,'C');
    $pdf->SetFont('Arial','B',10);
    $pdf->SetXY($x + 25, $y);
    $pdf->MultiCell(25, 12.5, 'Physical Science', 1,'C');
    $pdf->SetXY($x + 50, $y);
    $pdf->SetFont('Arial','B',8.5);
    $pdf->MultiCell(25, 8.3, 'Understanding Culture, Society and Politics', 1,'C');
    $pdf->SetFont('Arial','B',10);
    $pdf->SetXY($x + 75, $y);
    ...
    $pdf->SetXY($x + 100, $y);
    $pdf->MultiCell(25, 12.5, 'Research Project', 1,'C');
    $pdf->SetXY($x + 125, $y);
    $pdf->MultiCell(25, 12.5, 'Web Programming', 1,'C');
    $pdf->SetXY($x + 150, $y);
    $pdf->MultiCell(25, 8.3, 'Computer Servicing NCII', 1,'C');
    $pdf->SetXY($x + 175, $y);
    $pdf->SetFont('Arial','B',8.5);
    $pdf->MultiCell(25, 5, 'Work Immersion/ Career/ Advocacy/ Culminating', 1,'C');
    $pdf->SetFont('Arial','B',10);
    $pdf->SetXY($x + 200, $y);
    ...
    $pdf->Cell(25, 5, $rows['first_c_4'], 1,0, 'C');
    ...
    $pdf->Cell(25, 5, $rows['sec_first_ave'], 1,1, 'C');

    $pdf->SetFont('Arial','',11);
    $pdf->Cell(8, 10, '', 0,1, 'L');
    $pdf->Cell(8, 10, '', 0,0, 'L');
    $pdf->Cell(8, 10, 'The school welcomes you if you desire to know more about the progress of your child. Thank you and we hope that all is well with you and your family.', 0,0, 'L');
    $pdf->Cell(8, 10, '', 0,1, 'L');
    $pdf->Cell(8, 10, '', 0,1, 'L');
    $pdf->Cell(8, 10, '', 0,0, 'L');
...
elseif ($grading=="second") {
    $grading= "Fourth Quarterly Grade";
    $gradingetc= "Fourth Quarterly Academic Achievement/Progress Report";
    $sqlstudent = "SELECT * from ".$section."_grade_sec_sec WHERE student_first='$first' AND student_last='$last'";
    $stmt=mysqli_stmt_init($conn);
    if (!mysqli_stmt_prepare($stmt, $sqlstudent)) {
        echo "stmt";
        exit();
    ...
    */
    ...
    $pdf->Cell(25, 5, $rows['first_a_3'], 1,0, 'C');
    $pdf->Cell(25, 5, $rows['first_a_4'], 1,0, 'C');
    $pdf->Cell(25, 5, $rows['sec_sec_ave'], 1,1, 'C');
